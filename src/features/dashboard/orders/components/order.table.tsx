...
import { Order, OrderFormType } from '../types';
...
export default function OrderTable({ orders }: OrderTableProps) {
    ...
    const columns: ColumnDef<Order>[] = [
        ...
        },
        {
            ...
            cell: ({ row }) => {
                return <div className="flex items-center justify-center gap-2">
                    <span className="text-sm">Rp. {row.original.total.toLocaleString('id-ID')}</span>
                </div>
            },
        },
        {
            ...
            cell: ({ row }) => {
                let orderStatus = 'bg-gray-100 border border-gray-500 text-gray-500'
                if (row.original.payment_status === 'paid') {
                    orderStatus = 'bg-green-500 text-green-100'
                } else if (row.original.payment_status === 'unpaid') {
                    orderStatus = 'bg-red-500 text-red-100'
                }

                return (
                    <div className="flex items-center justify-center gap-2">
                        <span className={`capitalize text-sm ${orderStatus} px-2 py-1 rounded-md`}>{row.original.payment_status}</span>
                    </div>
                )
            },
        },
        ...
        },
        {
            ...
            cell: ({ row }) => {
                let orderStatus = 'bg-gray-100 border border-gray-500 text-gray-500'
                if (row.original.order_status === 'packing') {
                    orderStatus = 'bg-orange-100 border border-orange-500 text-orange-500'
                } else if (row.original.order_status === 'completed') {
                    orderStatus = 'bg-green-100 border border-green-500 text-green-500'
                } else if (row.original.order_status === 'cancelled') {
                    orderStatus = 'bg-red-100 border border-red-500 text-red-500'
                }

                return (
                    <div className="flex items-center justify-center gap-2">
                        <span className={`capitalize text-sm ${orderStatus} px-2 py-1 rounded-md`}>{row.original.order_status}</span>
                    </div>
                )
            },
        },
            ...
            cell: ({ row }) => (
                <div className="flex items-center justify-center gap-2">
                    <button
                        ...
                        onClick={() => openPopup(<HideConfirmation order={row.original} onChange={() => { }} />, 'max-w-lg')}
                    ...
                    <button
                        ...
                        onClick={() => openPopup(<EditOrder order={{ ...row.original, items: 1, total: 100000, payment_status: 'paid', order_status: 'packing', payment_method: 'cash', shipping: 'JNE', shipping_no: '1234567890', voucher: '1234567890', invoice_number: '1234567890', receipt_no: '1234567890' } as OrderFormType} />)}
                    >
                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div className="w-full overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 font-(family-name:--font-dm-sans)">
                <thead className="bg-gray-50">
                    {table.getHeaderGroups().map((headerGroup: HeaderGroup<Order>) => (
                        <tr key={headerGroup.id}>
                            {headerGroup.headers.map((header) => (
                                <th
                                    key={header.id}
                                    className="px-4 sm:px-6 py-3 text-left text-xs font-semibold text-gray-500 text-black tracking-wider cursor-pointer hover:bg-gray-100 whitespace-nowrap"
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {table.getRowModel().rows.map((row: Row<Order>, index: number) => (
