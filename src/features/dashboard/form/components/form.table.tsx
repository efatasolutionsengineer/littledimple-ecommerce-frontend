...
export default function FormTable({ forms }: FormTableProps) {
    ...
    const columns: ColumnDef<Form>[] = [
        ...
        },
        {
            ...
            header: 'Attachment',
            cell: ({ row }) => {
                return <div className="bg-gray-100 rounded-lg px-2 py-1 flex items-center justify-center gap-2">
                    {row.original.attachment ? <a href={row.original.attachment} target="_blank" rel="noopener noreferrer" className='flex items-center justify-center flex-wrap gap-2'>
                        <AttachmentIcon width={12} height={12} /> {row.original.attachment.length > 5 ? row.original.attachment.slice(0, 5) + '...' : row.original.attachment}
                    </a> : <span>-</span>}
                </div>
            },
        },
            ...
            cell: ({ row }) => (
                <div className="flex items-center justify-center gap-2">
                    <button
                        ...
                        onClick={() => openPopup(<EditForm form={row.original} />)}
                    >
        ...
        <div className="w-full overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200 font-(family-name:--font-dm-sans) text-black">
                <thead className="bg-gray-50">
                    {table.getHeaderGroups().map((headerGroup: HeaderGroup<Form>) => (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {table.getRowModel().rows.map((row: Row<Form>, index: number) => (
                        <tr key={row.id} className="text-center hover:bg-gray-50">
                            {row.getVisibleCells().map((cell: Cell<Form, unknown>) => (
                                <td
                                    key={cell.id}
