            ...
            cell: ({ row }) => {
                return (<div className="flex items-center justify-center gap-4">
                    <div className='size-10 rounded-lg overflow-hidden'>
                        ...
                        <img src={row.original.image} alt={row.original.name} width={50} height={50} loading="lazy" className="w-full h-full object-cover" />
                    </div>
                    <div className="text-left">
                        <p className="text-sm font-semibold mb-1 text-black">{row.original.name}</p>
                        <p className="text-sm text-gray-500">{row.original.size}</p>
                    </div>
                </div>
                )
            },
        },
        {
            ...
            cell: ({ row }) => {
                return <div className="flex items-center justify-center gap-2">
                    <span className="text-sm">Rp. {row.original.price.toLocaleString('id-ID')}</span>
                </div>
            },
        },
        {
            ...
            cell: ({ row }) => {
                return <div className="tetx-center">
                    <span className="text-sm mb-1 text-black font-semibold">{row.original.stock} Item Left</span>
                    <p className="text-sm text-gray-500">{row.original.sold ?? 0} Sold</p>
                </div>
            },
        },
        {
            ...
            cell: ({ row }) => {
                return (
                    <div className="flex items-center gap-2">
                        <div className="flex items-center gap-2 bg-[#F4F9FF] rounded-xl py-2 px-4">
                            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M12 17.27L18.18 21L16.54 13.97L22 9.24L14.81 8.63L12 2L9.19 8.63L2 9.24L7.45 13.97L5.82 21L12 17.27Z" fill="#FF9680" />
                            </svg>
                            <span className="font-semibold text-black">{row.original.rating ?? 0}</span>
                        </div>
                        <span className="text-black">
                            {row.original.rating ?? 0} Review
                        </span>
                    </div>
                );
            },
            ...
            cell: ({ row }) => (
                <div className="flex items-center justify-center gap-2">
                    <button
                        ...
                        onClick={() => openPopup(<HideConfirmation product={row.original} onChange={() => { }} />, 'max-w-lg')}
                    ...
                    <button
                        ...
                        onClick={() => openPopup(<EditProduct product={row.original} />)}
                    >
                        <svg width="16" height="17" viewBox="0 0 16 17" fill="none" xmlns="http://www.w3.org/2000/svg">
