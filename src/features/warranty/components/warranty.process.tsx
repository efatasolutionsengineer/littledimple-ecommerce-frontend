...
export default function WarrantyProcess() {
    ...
    const [data,] = useState<WarrantyProcess[]>([
        {
            no: 1,
        ...
        },
        {
            ...
            customer: "John Doe",
            price: 300000,
            warranty_date: "2021-01-03",
            status: "pending"
        }
    ]); // You'll need to populate this with actual data
    const columns: ColumnDef<WarrantyProcess>[] = [
        {
            accessorKey: 'no',
            header: 'No',
        },
    ...
    return (
        <div className="w-full overflow-x-auto mt-16">
            <h4 className='text-hijau-tua text-center text-3xl mb-8'>Warranty Process</h4>
            <table className="min-w-full divide-y divide-gray-200 font-(family-name:--font-dm-sans)">
                <thead className="bg-gray-50">
                    {table.getHeaderGroups().map((headerGroup: HeaderGroup<WarrantyProcess>) => (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {table.getRowModel().rows.map((row: Row<WarrantyProcess>) => (
                        <tr key={row.id} className="text-center hover:bg-gray-50">
                            {row.getVisibleCells().map((cell: Cell<WarrantyProcess, unknown>) => (
                                <td
                                    key={cell.id}
                                    className="px-6 py-4 whitespace-nowrap text-sm text-gray-500"
                                >
                                    {cell.column.id === 'status' ? <span className={row.original.status === 'completed' ? 'capitalize text-green-500 bg-green-100 border border-green-500 rounded-lg px-3 py-1' : 'capitalize text-red-500 bg-red-100 border border-red-500 rounded-lg px-2 py-1'}>{row.original.status}</span> : flexRender(cell.column.columnDef.cell, cell.getContext())}
                                </td>
                            ))}
                        </tr>
                    ))}
