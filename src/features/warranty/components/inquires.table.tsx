...
interface InquiresTableProps {
    inquires: Inquire[];
}

export default function InquiresTable({ inquires }: InquiresTableProps) {
    ...
    return (
        <div className="w-full overflow-x-auto mt-16">
            <h4 className='text-hijau-tua text-center text-3xl mb-8'>Inquires</h4>
            <table className="min-w-full divide-y divide-gray-200 font-(family-name:--font-dm-sans)">
                <thead className="bg-gray-50">
                    {table.getHeaderGroups().map((headerGroup: HeaderGroup<Inquire>) => (
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                    {table.getRowModel().rows.map((row: Row<Inquire>, index: number) => (
                        <tr key={row.id} className="text-center hover:bg-gray-50">
                            {row.getVisibleCells().map((cell: Cell<Inquire, unknown>) => (
                                <td
                                    key={cell.id}
